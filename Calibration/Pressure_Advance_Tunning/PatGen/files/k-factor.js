...
function genGcode() {
  ...
  var PRINTER = $('#PRINTER').val(),
    ...
    ACCELERATION_X = parseInt($('#PRINT_ACCL_X').val()),
    ACCELERATION_Y = parseInt($('#PRINT_ACCL_Y').val()),
    ACCELERATION_Z = parseInt($('#PRINT_ACCL_Z').val()),
    ACCELERATION_E = parseInt($('#PRINT_ACCL_E').val()),
  ...
  var k_script = '; ### Marlin K-Factor Calibration Pattern ###\n' +
    ...
    '; Printing Acceleration X = ' + ACCELERATION_X + ' mm/s^2\n' +
    '; Printing Acceleration Y = ' + ACCELERATION_Y + ' mm/s^2\n' +
    '; Printing Acceleration Z = ' + ACCELERATION_Z + ' mm/s^2\n' +
    '; Printing Acceleration E = ' + ACCELERATION_E + ' mm/s^2\n' +
    ...
    'G1 Z5 F100 ; Z raise\n' +
    'M140 S' + BED_TEMP + ' ; Set bed temperature (wait)\n' +
    'M190 S' + BED_TEMP + ' ; Wait for bed temp\n'+
    'G4 S30 ; wait for minute to warmup glass bed' +
    ...
    'M201 X' + ACCELERATION_X +' Y' + ACCELERATION_Y + ' Z' + ACCELERATION_Z + ' E' + ACCELERATION_E + ' ; sets maximum accelerations, mm/sec^2' +
    'M203 X150 Y150 Z15 E120 ; sets maximum feedrates, mm/sec' +
    'M205 J0.039 ; set junction deviation' +
    'M205 S0 T0 ; sets the minimum extruding and travel feed rate, mm/sec' +
    'M204 P' + ACCELERATION_E + ' ; Acceleration\n' +
    'M204 S' + ACCELERATION_X + ' ; adjust acceleration' +
  ...
  if (USE_PRIME) {
    ...
    k_script += ';\n' +
      ...
      createLine(primeStartX, primeStartY + PRINT_SIZE_Y, PRINT_SIZE_Y, basicSettings, {
        'extMult': EXT_MULT_PRIME,
        'speed': SPEED_PRIME
      }) +
      moveTo(primeStartX + (LINE_WIDTH * 1.5), primeStartY + PRINT_SIZE_Y, basicSettings) +
      createLine(primeStartX + (LINE_WIDTH * 1.5), primeStartY, -PRINT_SIZE_Y, basicSettings, {
        'extMult': EXT_MULT_PRIME,
        'speed': SPEED_PRIME
      }) +
  ...
  if (USE_FRAME) {
    ...
    k_script += ';\n' +
      ...
      createLine(frameStartX1, frameStartY + frameLength, frameLength, basicSettings, {
        'extMult': EXT_MULT * 1.1
      }) +
      moveTo(frameStartX1 + LINE_WIDTH, frameStartY + frameLength, basicSettings) +
      createLine(frameStartX1 + LINE_WIDTH, frameStartY, -frameLength, basicSettings, {
        'extMult': EXT_MULT * 1.1
      }) +
      doEfeed('-', basicSettings, (USE_FWR ? 'FWR' : 'STD')) +
  ...
  for (var i = patSettings['kStart']; i <= patSettings['kEnd']; i += patSettings['kStep']) {
    if (k % 2 === 0) {
      gcode += 'M900 K' + Math.round10(i, -3) + ' ; set K-factor\n' +
        ...
        createLine(startX + patSettings['lengthSlow'], startY + j, patSettings['lengthSlow'], basicSettings, {
          'speed': basicSettings['slow']
        }) +
        createLine(startX + patSettings['lengthSlow'] + patSettings['lengthFast'], startY + j, patSettings['lengthFast'], basicSettings, {
          'speed': basicSettings['fast']
        }) +
        createLine(startX + (2 * patSettings['lengthSlow']) + patSettings['lengthFast'], startY + j, patSettings['lengthSlow'], basicSettings, {
          'speed': basicSettings['slow']
        }) +
        createLine(startX + (2 * patSettings['lengthSlow']) + patSettings['lengthFast'], startY + j + patSettings['lineSpacing'], patSettings['lineSpacing'], basicSettings, {
          'speed': basicSettings['fast']
        });
      ...
      gcode += 'M900 K' + Math.round10(i, -3) + ' ; set K-factor\n' +
        'M117 K' + Math.round10(i, -3) + ' ; \n' +
        createLine(startX + patSettings['lengthSlow'] + patSettings['lengthFast'], startY + j, patSettings['lengthSlow'], basicSettings, {
          'speed': basicSettings['slow']
        }) +
        createLine(startX + patSettings['lengthSlow'], startY + j, patSettings['lengthFast'], basicSettings, {
          'speed': basicSettings['fast']
        }) +
        createLine(startX, startY + j, patSettings['lengthSlow'], basicSettings, {
          'speed': basicSettings['slow']
        }) +
        createLine(startX, startY + j + patSettings['lineSpacing'], patSettings['lineSpacing'], basicSettings, {
          'speed': basicSettings['fast']
        });
      j += patSettings['lineSpacing'];
      k += 1;
  ...
  for (var i = 0, len = sNumber.length; i < len; i += 1) {
    for (var key in glyphSeg[sNumber.charAt(i)]) {
      if (glyphSeg[sNumber.charAt(i)].hasOwnProperty(key)) {
        var up = createLine(startX + (xCount * glyphSegHeight), startY + (yCount * glyphSegHeight) + glyphSegHeight, glyphSegHeight, basicSettings, {
            'speed': basicSettings['slow'],
            'comment': ' ; ' + sNumber.charAt(i) + '\n'
          }),
          down = createLine(startX + (xCount * glyphSegHeight), startY + (yCount * glyphSegHeight) - glyphSegHeight, glyphSegHeight, basicSettings, {
            'speed': basicSettings['slow'],
            'comment': ' ; ' + sNumber.charAt(i) + '\n'
          }),
          right = createLine(startX + (xCount * glyphSegHeight) + glyphSegHeight, startY + (yCount * glyphSegHeight), glyphSegHeight, basicSettings, {
            'speed': basicSettings['slow'],
            'comment': ' ; ' + sNumber.charAt(i) + '\n'
          }),
          left = createLine(startX + (xCount * glyphSegHeight) - glyphSegHeight, startY + (yCount * glyphSegHeight), glyphSegHeight, basicSettings, {
            'speed': basicSettings['slow'],
            'comment': ' ; ' + sNumber.charAt(i) + '\n'
          }),
          mup = moveTo(startX + (xCount * glyphSegHeight), startY + (yCount * glyphSegHeight) + glyphSegHeight, basicSettings),
          dot = createLine(startX, startY + glyphSegHeight2, glyphSegHeight2, basicSettings, {
            speed: basicSettings['slow'],
            comment: ' ; dot\n'
          });
...
$(window).load(() => {
  ...
  if (lsSettings !== undefined) {
    ...
    if (!settings['Version'] || settings['Version'] != SETTINGS_VERSION) {
      ...
      $('#PRINT_ACCL_X').val(settings['ACCELERATION_X']);
      $('#PRINT_ACCL_Y').val(settings['ACCELERATION_Y']);
      $('#PRINT_ACCL_Z').val(settings['ACCELERATION_Z']);
      $('#PRINT_ACCL_E').val(settings['ACCELERATION_E']);
