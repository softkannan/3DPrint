...
from ... import config
from .hexa_mah import *
app = adsk.core.Application.get()
ui = app.userInterface


# TODO *** Specify the command identity information. ***
CMD_ID = f'{config.COMPANY_NAME}_{config.ADDIN_NAME}_hexaMidAirHole'
CMD_NAME = 'Hexagon mid air hole'
CMD_Description = 'Hexagon mid air hole for supportless 3d printing.'
...
template_list = {'Custom':(3.41,1.3,3.2,2), 'M2':(4.32,1.6,4,2.5), 'M2.5':(5.45,2,5,3), 'M3':(6.01,2.4,5.5,3.5), 'M4':(7.66,3.2,7.0,4.5), 'M5':(8.79,4.7,8,5.5), 'M6':(11.05,5.2,10,6/5)}
...
def command_created(args: adsk.core.CommandCreatedEventArgs):
    ...
    inputs.addValueInput('hexa_dist', 'Vertices distance', 'mm',  adsk.core.ValueInput.createByReal(0.2))
    inputs.addValueInput('hexa_depth', 'Hexagon depth', 'mm',  adsk.core.ValueInput.createByReal(0.1))
...
def command_execute(args: adsk.core.CommandEventArgs):
    ...
    hexa_dist: adsk.core.ValueCommandInput = inputs.itemById('hexa_dist')
    hexa_depth: adsk.core.ValueCommandInput = inputs.itemById('hexa_depth')
    ...
    tool_body = create_hexa_mah_tool(comp, layer_thick.value, hexa_dist.value, hexa_depth.value, small_hole.value, small_depth.value)
    for ent in ents:
        ...
        if type(ent) is adsk.fusion.SketchPoint:
            ...
            cut_hexa_mah_by_point(target_body, tool_body, point)
        elif type(ent) is adsk.fusion.SketchCircle:
            futil.log('target is SketchCircle')
            circle = adsk.fusion.SketchCircle.cast(ent)
            cut_hexa_mah_by_point(target_body, tool_body, circle.centerSketchPoint)
            ...
            if (isinstance(edge.geometry, adsk.core.Circle3D)):
                ...
                cut_hexa_mah_by_3dcircle(target_body, tool_body, edge.geometry)
    # not visible
...
def command_input_changed(args: adsk.core.InputChangedEventArgs):
    ...
    hexa_depth: adsk.core.ValueCommandInput = inputs.itemById('hexa_depth')
    small_hole: adsk.core.ValueCommandInput = inputs.itemById('small_hole')
    hexa_dist.value = arr[0] / 10.0
    hexa_depth.value = arr[1] / 10.0
    small_hole.value = arr[3] / 10.0
...
def command_validate_input(args: adsk.core.ValidateInputsEventArgs):
    ...
    if layer_thick.value < 0:
        enflag = False
    if hexa_dist.value <= 0:
        enflag = False
    if hexa_depth.value < 0:
        enflag = False
